    ...
    <div class="min-h-[80vh] flex items-center justify-center px-4">
      <div class="w-full max-w-md">
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
          ...
          <p class="text-gray-500 text-sm mb-8">Inicia sesión en tu cuenta Love4Sneakers</p>
          ...
          <form (ngSubmit)="submit()" class="space-y-4">
            <div>
              ...
              <input id="login-email" type="email" [(ngModel)]="email" name="email" required
            ...
            <div>
              ...
              <input id="login-password" type="password" [(ngModel)]="password" name="password" required
                class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-400 transition-colors" />
            </div>
          ...
          <div class="relative my-6">
            ...
            <div class="relative flex justify-center"><span class="bg-white px-4 text-xs text-gray-400">o continúa con</span></div>
          </div>

          <button (click)="signInGoogle()"
            class="w-full border border-gray-200 hover:bg-gray-50 text-gray-700 font-medium py-3 rounded-xl flex items-center justify-center gap-3 transition-colors text-sm">
          ...
          <p class="text-center text-sm text-gray-500 mt-6">
            ¿No tienes cuenta? <a routerLink="/auth/register" class="text-indigo-600 hover:text-indigo-800 font-medium">Regístrate</a>
          </p>
        </div>
      </div>
    </div>
...
export class LoginPage {
  ...
  async signInGoogle(): Promise<void> {
    try {
      ...
      this.error.set((e as { message?: string }).message ?? 'Error con Google');
    }
  }
}
