...
describe('AdminGuard', () => {
  ...
  it('throws ForbiddenException when request has no supabaseUser', async () => {
    await expect(guard.canActivate(makeCtx(null))).rejects.toThrow(
      ForbiddenException,
    );
  ...
  it('throws ForbiddenException when user does not exist in the database', async () => {
    ...
    await expect(guard.canActivate(makeCtx({ id: 'ghost' }))).rejects.toThrow(
      ForbiddenException,
    );
  });
  ...
  it('returns true for an active ADMIN', async () => {
    ...
    await expect(
      guard.canActivate(makeCtx({ id: 'admin-uuid' })),
    ).resolves.toBe(true);
  });
});
